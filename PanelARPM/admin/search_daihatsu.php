...
if($_SERVER['REQUEST_METHOD']=='POST') {
  $search = $_POST['search'];
  $sql = "SELECT * FROM tbl_daihatsu WHERE harga_mobil_daihatsu <= $search ORDER BY data_topsis_daihatsu ASC";
  $res = mysqli_query($con,$sql);
  $result = array();
  while($row = mysqli_fetch_array($res)){
    array_push($result, array('id_mobil_daihatsu'=>$row[0], 'link_img_daihatsu'=>$row[1], 'nama_mobil_daihatsu'=>$row[2], 
